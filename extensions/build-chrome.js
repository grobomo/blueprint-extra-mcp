...
// Copy vanilla JS source files and fix import paths
// Source uses ../../shared/ (relative to extensions/chrome/src/)
// Dist uses ../shared/ (relative to dist/chrome/src/)
for (const srcFile of ['background-module.js', 'content-script.js']) {
  let content = fs.readFileSync(path.join(chromeSrc, 'src', srcFile), 'utf8');
  content = content.replace(/from\s+['"]\.\.\/\.\.\/shared\//g, "from '../shared/");
  content = content.replace(/import\s+['"]\.\.\/\.\.\/shared\//g, "import '../shared/");
  fs.writeFileSync(path.join(distSrcDir, srcFile), content);
}
