...
class UnifiedBackend {
  ...
  async _handleBrowserTabs(args, options = {}) {
    ...
    if (action === 'list') {
      ...
      const tabList = windowIds.map(windowId => {
        ...
        // Fork extra: detect incognito windows
        const hasIncognito = windowTabs.some(tab => tab.incognito);
        const windowLabel = hasIncognito ? 'INCOGNITO' : '';
        const focusLabel = windowId == result.focusedWindowId ? 'FOCUSED' : '';
        const labels = [focusLabel, windowLabel].filter(Boolean).join(', ');
        const windowHeader = labels ?
          `\n**Window ${windowId} (${labels}):**\n` :
          `\n**Window ${windowId}:**\n`;

        const tabLines = windowTabs.map(tab => {
          const markers = [];
          if (tab.active) markers.push('ACTIVE');
          if (tab.incognito) markers.push('INCOGNITO');
  ...
  async _handleSnapshot(options = {}) {
    ...
    // Fork extra: extract same-origin iframe content
    let iframeContent = '';
    try {
      const iframeResult = await this._transport.sendCommand('forwardCDPCommand', {
        method: 'Runtime.evaluate',
        params: {
          expression: `(function() {
            var results = [];
            ...
            for (var i = 0; i < iframes.length; i++) {
              try {
                ...
                if (!doc) continue;
                var id = iframes[i].id || iframes[i].name || ('iframe-' + i);
                var text = doc.body ? doc.body.innerText : '';
                if (text && text.trim().length > 0) {
                  var truncated = text.length > 5000 ? text.substring(0, 5000) + '...(truncated)' : text;
                  results.push('--- iframe: ' + id + ' (' + doc.querySelectorAll('*').length + ' elements) ---\\n' + truncated);
                }
      ...
      if (iframeResult?.result?.value) {
        iframeContent = '\n\n### Same-Origin Iframe Content\n\n' + iframeResult.result.value;
      }
    } catch (e) {
      // Non-fatal -- iframe extraction is best-effort
      debugLog('Iframe content extraction failed:', e.message);
    }
    ...
    return {
      content: [{
        type: 'text',
        text: `### Page Snapshot\n\n${snapshot}${truncationMessage}${iframeContent}`
      }],
      isError: false
    };
  }
